...
async def pawn_move(query: CallbackQuery):
	...
	# ход
		...
		if pawn_sym == ('🔹' if team == 'red' else '🔺'):
			...
			if team == 'red':
				await query.answer("Синий замок разрушен!")
				blue_alive = False
			else:
				await query.answer("Красный замок разрушен!")
				red_alive = False
				
		if not red_alive and count_pawns('red') == 0:
			win = 'red'
			await alert_win(query)
			return
		elif not blue_alive and count_pawns('blue') == 0:
			win == 'blue'
			await alert_win(query)
			return
